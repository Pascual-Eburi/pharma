...
class ClientesModel extends UsuariosModel{
    // nombre de la tabla
    protected $nombre_tabla = 'cliente';
    public function getDataClientes($id = 0){
        ...
        try{
            $sql = "CALL infoClientes(:id)";
            $sentencia = $this->conexion->prepare($sql);
            $sentencia->bindParam(':id', $id, PDO::PARAM_STR, 3);
            $sentencia->execute();

            if($sentencia->rowCount() <= 0){
                $resultado['data'] = 'No hay registros de clientes';
                return $resultado;
            }
    ...
    public function eliminarMultiple($registros = null){
        ...
        try{
            ...
            $query_usuario = $this->conexion->prepare("SELECT id_usuario FROM {$this->nombre_tabla} WHERE id_cliente = ? ");
            ...
            $sql = "DELETE FROM {$this->nombre_tabla} WHERE id_cliente = ?";
