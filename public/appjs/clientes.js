import {mostrarNotificaciones, capitalizarPrimeraLetra, contieneSoloNumeros, inicializarTablaDatos, validarInput, ordenarArray, generarAvatar, formatDate} from './modules.js';
...
$(document).ready(function(){
    ...
    app.tablaDatos = tablaClientes; // metodo set: tabla de datos
    app.formulario = $('#addUpdate form');
    app.urlPeticiones = 'clientes';
    ...
    $(document).click(function(event){
        ...
        */
        if( $elemento.parents('.botonEditar').length > 0 || $elemento.hasClass('botonEditar') ){
            ...
            if($elemento.parents('.botonEditar').attr('data-cliente')){
                $id = $elemento.parents('.botonEditar').attr('data-cliente');
            }
            if($elemento.hasClass('botonEditar')){
                $id = $elemento.attr('data-cliente');
            }
            
            //funcion editar
            if($id && $id > 0 ){editarCliente($id);  } // llama a la funcion para editar  
            return 
        } // fin if botonEditar
        ...
        if($elemento.parents('.botonEliminar').length > 0 || $elemento.hasClass('botonEliminar')){
            event.preventDefault();
            if($elemento.parents('.botonEliminar').attr('data-cliente')){$id = $elemento.parents('.botonEliminar').attr('data-cliente') ; }
            if($elemento.hasClass('botonEliminar')){$id = $elemento.attr('data-cliente');}

            // eliminar pais;
            if($id && $id > 0 ){eliminarCliente($id);} // delete.
    ...
    //--------------- COMUN ------------------------
/*
    console.time('SIN_STORED');
    $.ajax({
        url: 'clientes',
        method: 'post',
        data:{accion:'traer-registros', registro: '0'},
        dataType: 'json',
        cache: true,
        success: function(data){
            console.log(data)
            console.timeEnd('SIN_STORED')
        },
...
document.addEventListener("DOMContentLoaded", async function () { 
    app.inicializarDatePickers();
   let selectProvincias = await app.cargarDatosSelectProvincias();
    let selectRoles = await app.cargarDatosSelectRoles('7');
...
modalAddUpdate.addEventListener('hide.bs.offcanvas', ()=>{
    ...
    resetRenderInfoCliente(); // reset render info usuario : si id form es editar
});
...
----------------------------------------------------*/
const addBtn = document.getElementById('registrarCliente');
...
/*---------------------------------------------------------------
            TRAER Y RENDERIZAR Cliente SELECCIONADO 
-----------------------------------------............................*/
const renderizarInfoCliente = async($id = null) =>{
    if(!$id || !contieneSoloNumeros($id)){
        console.warn('Id de cliente no valido...');
        return false;
    }
    ...
    console.log('DIA:', dia, 'MES: ', mes + 1, 'AÑO:', year)

    return*/
    // renderizar info en el formulario para editar
    // info.data: contiene la data del cliente
    
    /*------------------------ ID USUARIO Y cliente -------------- */
    ...
    if(!$id_cliente){
        $form.insertAdjacentHTML('beforeend','<input type="hidden" class="input" name="id_cliente" id="id_cliente" data-required="true" data-min-length="1" data-max-length="6" data-format="numbers"/>')
    }
    ...
    let infoCliente = new Map(); // datos o info del reg.  seleccionado
    const excluir = ['pedidos', 'roll', 'provincia', 'ccaa', 'fecha_registro', 'edad'];
    for (let clave in data){ 
        // quitar las claves que no se van a usar
        if( !(excluir.includes(clave)) ){
            if((clave == 'foto' && !data[clave]) || (clave == 'telefono' && !data[clave]) ){
                //continue;
                infoCliente.set(clave, ''); // llenar Map datosRegistro 
            }else if(clave == 'id_provincia' && !data['id_provincia']){
                infoCliente.set(clave, '0'); // llenar Map datosRegistro 
            }else if(clave == 'id_roll' && !data['id_roll']){
                infoCliente.set(clave, '0'); // llenar Map datosRegistro 
            }else if(clave == 'fechaNacimiento'){
                (data[clave]) ? infoCliente.set(clave, formatDate(data[clave], 'dd-mm-yyyy')) : infoCliente.set(clave, ' ');
            }else{
                infoCliente.set(clave, `${data[clave]}`); // llenar Map datosRegistro 
            }
        } 
    }
    // guardar la info del registro en la app
    app.infoRegistro = infoCliente; 
...
const resetRenderInfoCliente = () =>{
    ...
    if(id_form == 'editarRegistro'){
        ...
        if($id_cliente) $id_cliente.remove();
        ...
        /*------------------------ TITULO VENTANA Y SUBTITULO -------------- */
        document.querySelector('#addUpdate .header').textContent = `Registra un nuevo cliente`;
        document.querySelector('#addUpdate .row-header p').textContent = 'Rellena el formulario para registrar al cliente';
...
/*----------------------------------------------
            ELIMINAR CLIENTE
----------------------------------------------*/
const eliminarCliente = async(id = null) =>{
    if(!id || !contieneSoloNumeros(id)){
        mostrarNotificaciones({
            tipo: 'aviso',
            titulo: 'ID no valido',
