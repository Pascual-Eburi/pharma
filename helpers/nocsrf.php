...
class NoCSRF {
    ...
    public static function check( $key, $origin, $throwException=false, $timespan=null, $multiple=false ){
        ...
        if ( $origin[ $key ] != $hash )
            if($throwException)
                ...
                return [$origin, $hash, $origin[$key]];

        // Check for token expiration
        if ( $timespan != null && is_int( $timespan ) && intval( substr( base64_decode( $hash ), 0, 10 ) ) + $timespan < time()*10 )
            if($throwException)
    ...
    public static function generate( $key )
    {
        ...
        $token = base64_encode( time()*10 . $extra . self::randomString( 32 ) );
